...
Lash, Claude and Codex remain supported execution choices. The coordinator runs the TypeScript RLM dialect and may define arbitrary Lash processes, attach them to cron, timer, or typed Thread-event triggers, and call ordinary Hirsel tools from their bodies. Hirsel contributes trigger sources and projects the Lash registry; it does not wrap process execution in another engine. Every process has a concrete owning Thread. Wakes and terminal results become bounded conversation messages and follow the same fork-triage rule as other non-owner input. Registered trigger and process state reopens from Lash's durable stores after restart. Recurring processes are created only at an Owner's request.

The native Lash coding worker remains a deliberately narrow standard tool-calling session. Its read/edit/write/command profile has neither the RLM dialect nor process and trigger orchestration; those capabilities belong to the coordinator. Background work requires an explicit destination. Per-Thread FIFO and bounded cross-Thread concurrency preserve request identity. Archive and snooze pause automatic report-triggered execution while retaining reports for later delivery; settlement is independent.
