...
// Component-level gates for the Side Chat sheet (ADR-0008): a host-opened side
// chat renders, the conclude flow reaches confirm_conclusion with the Owner's edited
...
/** Mounts ChatView with a host-opened SideChatSheet and a fake ws client that behaves like the
...
describe("Host-opened Side Chat sheet", () => {
...
describe("Discard requires its confirm", () => {
  ...
  it("Discard (confirmed) closes the sheet with no conclusion; the item stays open", async () => {
    ...
    expect(store.state.sideChats["side:1"]).toBeUndefined();
  });
});
