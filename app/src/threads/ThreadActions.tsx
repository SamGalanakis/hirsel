...
export function ThreadActions(props: { thread: Thread; quick?: boolean; now?: number }) {
  ...
  return <div class="flex shrink-0 items-start">
    <Show when={props.quick && props.thread.kind === "task"}><button class={`${control} hidden min-[420px]:inline-flex`} aria-label={`${props.thread.settled_at ? "Reopen" : "Mark done"} ${props.thread.title}`} title={props.thread.settled_at ? "Reopen task" : "Mark task done"} onClick={() => actions().find(action => action.id === "settle")?.run()}><ThreadActionSymbol name={props.thread.settled_at ? "reopen" : "settle"} /></button></Show>
    <DropdownMenu>
