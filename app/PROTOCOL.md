...
Each Thread carries `own_headline`, displayed `headline`, `previous_headline`, `headline_revision`, `last_seen_headline_revision`, and `{kind,reason}` status. Status kinds are `needs_you`, `running`, `queued`, `hung`, `sleeping`, and `idle`. `mark_thread_headlines_seen {client_id,history_id,thread_ids}` advances only the displayed rows' headline baselines and acknowledges through `thread_action_applied`; it never changes `read`.

Reply pills are not protocol records. The web client derives them from complete, non-truncated `tool_start`/`tool_done` timeline payloads and existing refusal activities. Malformed payloads are ignored.
...
Selection uses an explicit route first. An unavailable explicit route remains unavailable rather than selecting another Thread. Route-free `/` restores the active top-level Space in `hirsel.last-space.<history_id>`, otherwise selects the lowest-ID active top-level Space, otherwise creates Home through ordinary `create_thread`. `/t/0?history={uuid}` remains an ordinary route. Portable links require their history UUID; malformed, wrong-history and missing destinations have no recipient. The incoming URL survives a history reset until validated by the current hello. Cached IDs do not authorize navigation or sending before that handshake. Native push destinations carry history and Thread identity and reject obsolete histories.

The composer labels the recipient and says plainly when that recipient is a Task worker. `focusedId` is navigation only. “Talk about this” selects the containing Space chat and inserts `#<task-id>` in its draft; the reference carries no snapshot and changes no reach.
...
The Host uses one canonical storage schema 18: symbol/image Thread icons whose
