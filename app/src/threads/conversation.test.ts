...
describe("authoritative conversation chronology", () => {
  ...
  it("moves activated work to started_at so a later turn cannot jump ahead of an earlier reply", () => {
    ...
    const activated = { ...queued, state: "running" as const, started_at: "2026-09-10T15:29:25.089Z" };
    expect(conversationEntries({ ...history, turns: [first, activated] }).map(row => row.key)).toEqual(["message-1", "message-2", "turn-12", "turn-13"]);
  });
