...
# See runbook.md "Known limitation" and e2e/protocol-compatibility/RULES.md. Flips to a real pass when lash #8 is fixed.
if wait_agent_message_after "$before" '(.body | contains("GREEN-742"))' 60; then
  ...
  printf 'KNOWN-FAIL post-compaction recall did not return GREEN-742 — upstream lash #8 (continue_as seed drop). See e2e/protocol-compatibility/compaction/runbook.md.\n' >&2
fi
