...
> Historical protocol-coverage snapshot. The composite scenario described in the original report
> has since been retired. Current cross-surface coverage is the deterministic Task Margins browser
> runner documented in [`../RULES.md`](../RULES.md), with evidence in
> [`../reports/task-margins-latest.md`](../reports/task-margins-latest.md).

## Coverage Audit

| Scenario / product surface | Covered where | Gap before this pass | Result after this pass |
| --- | --- | --- | --- |
| Attachment protocol | `e2e/protocol-compatibility/attachments` | Only proved upload/replay/fetch and scripted stored-path notes. | Added `e2e/protocol-compatibility/attachment-agent-behavior` for real vision plus text-file tool use. |
| Real Sub-agent delegation | `e2e/protocol-compatibility/delegation-loop` used fake driver only. | No end-to-end real Codex Driver coverage. | Added `e2e/protocol-compatibility/real-subagent`; real Codex fixed a repo and interruption reached `cancelled`. |
| Abandoned process recovery | None. | ADR-0004 promise was untested. | Added `e2e/protocol-compatibility/abandoned-recovery`; no respawn passed, abandoned visibility failed. |
| Multi-turn conversation memory | `e2e/protocol-compatibility/restart-persistence` only asked one-off exact replies across restarts. | No ordinary multi-turn recall before and after restart. | Added `e2e/protocol-compatibility/multi-turn-memory`; recall passed before and after restart. |
| Compaction | None. | `prompts/agent.md` documents `control.continue_as`, but no runbook checked it. | Added `e2e/protocol-compatibility/compaction`; `continue_as` was visible, but post-compaction recall failed. |
| Ping read state | `e2e/protocol-compatibility/ping-read` | Read/unread only; no reply/resolution lifecycle. | Added `e2e/protocol-compatibility/pings-lifecycle`; reply, Owner resolution, and Agent resolution are covered. |
| Delegation loop | `e2e/protocol-compatibility/delegation-loop` | Covered fake-driver terminal and Quick Reply flow. | Still covered; real-driver gap moved to `real-subagent`. |
| Send/queue/cancel | `e2e/protocol-compatibility/send-queue-cancel` | Covered send injection, next-turn queueing, queued cancel, active-turn cancel. | No new gap found in this pass. |
| Monitors | `e2e/protocol-compatibility/monitors` | Covered debug-created and Agent-created monitors, wake, restart. | No new gap found in this pass. |
| Timers | `e2e/protocol-compatibility/timers` | Covered one-shot timer wake. | No new gap found in this pass. |
| Turn timeline and tool visibility | `e2e/protocol-compatibility/turn-timeline` | Covered ordered `turn_event` and tool summaries. | New scenarios also exercise `continue_as`, `shell_run`, `subagents_*`, and `pings_resolve` visibility. |
...
which surface an output lands on, how the Agent delegates, when it interrupts. Four real-Agent
runbooks were authored to assert those directly; the current deterministic composite browser suite
is listed alongside them below. The judgment runbooks require Codex OAuth (`~/.codex/auth.json`) and
...
| `task-margins` | Global conversation, Task dives, adaptive instruments, utilities, and responsive keyboard behavior compose without context loss. | The current browser runner records all objective gates and outgoing frames in one deterministic report. |
