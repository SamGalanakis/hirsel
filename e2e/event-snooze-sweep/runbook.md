...
Standard neutral-workdir boot (see `e2e/RULES.md`): scripted agent, fake driver, `/tmp` data dir,
verified-free port, `HIRSEL_DEBUG=1`. Source `e2e/lib/runbook-lib.sh` and use `start_hirsel_host`.
...
node "$REPO/e2e/event-snooze-sweep/clear-finished.cjs" "ws://127.0.0.1:$PORT/ws" "$HIRSEL_TOKEN"
...
Per `e2e/RULES.md`: record the event ids, the exact rejected-snooze error body, the `snoozed_until`
