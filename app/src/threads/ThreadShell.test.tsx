...
describe("thread workspace", () => {
  ...
  it("does not invite starting an empty conversation while a turn is running", () => {
    ...
    expect(view.getByRole("button", { name: "Stop the agent" })).toBeInTheDocument();
    expect(view.getByRole("button", { name: "Send after current turn" })).toBeVisible();
  });
  it("retries failed conversation reads without losing the addressed draft", async () => {
    ...
    flush(() => handleThreadMessage({ type: "thread_opened", client_id: retryRequest.client_id, detail: { brief: { text: "", artifact_ids: [] }, thread: makeThread(1), messages: [], activities: [], turns: [], effects: [], turn_timelines: [], related_items: [], grants: [], has_more: false } }));
  ...
  it("talks about a Task in its Space chat with one explicit bounded focus", async () => {
    flush(() => setThreadState(draft => {
      ...
      draft.histories[1] = {
        brief: { text: "Accepted Task brief", artifact_ids: [] },
        messages: [],
        turns: [],
        activities: [],
        loaded: true,
  ...
  it("keeps owner-facing summaries inline and raw records as plain trace rows", () => {
    ...
    expect(screen.getByRole("textbox", { name: "Step in with worker Buy groceries" })).toBeInTheDocument();
  });
  ...
  it("browses all artifacts as a utility pane and returns to the addressed composer", () => {
    const screen = render(() => <ThreadShell />);
    fireEvent.input(screen.getByRole("textbox", { name: "Step in with worker Buy groceries" }), { target: { value: "Follow up in groceries" } });
    ...
    expect(screen.queryByRole("textbox", { name: "Step in with worker Buy groceries" })).toBeNull();
    ...
    expect(screen.getByRole("textbox", { name: "Step in with worker Buy groceries" })).toHaveValue("Follow up in groceries");
    fireEvent.click(screen.getByRole("button", { name: "Send" }));
...
it("renders the exact persisted timeline from a fresh open_thread snapshot", async () => {
  ...
  flush(() => handleThreadMessage({ type: "thread_opened", client_id: frame.client_id, detail: {
    ...
    messages: [owner, agent], turns: [turn], effects: [],
...
it("keeps current brief reachable beyond the visible history page without moving its assignment", async () => {
  ...
  flush(() => handleThreadMessage({ type: "thread_opened", client_id: frame.client_id, detail: {
    ...
    messages: [{ id: 100, thread_id: 1, author: "owner", body: "Latest conversation", ref: null, ts: "2026-09-10T10:00:00Z" }], turns: [], effects: [], turn_timelines: [], activities: [assignment],
  } }));
