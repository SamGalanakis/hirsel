...
fn top_level(c: &rusqlite::Connection, id: u64) -> anyhow::Result<u64> {
    Ok(c.query_row("WITH RECURSIVE chain(id,parent_thread_id) AS (SELECT id,parent_thread_id FROM threads WHERE id=?1 UNION ALL SELECT t.id,t.parent_thread_id FROM threads t JOIN chain c ON c.parent_thread_id=t.id) SELECT id FROM chain WHERE parent_thread_id IS NULL", [id], |row| row.get(0))?)
}
...
fn outside_change(
    ...
    what: &str,
    now: &str,
) -> anyhow::Result<Option<hirsel_proto::ThreadActivity>> {
    let caller_root = top_level(c, caller.thread_id)?;
    let target_root = top_level(c, target_id)?;
    if caller_root == target_root || threads::get(c, target_root)?.kind != ThreadKind::Space {
        return Ok(None);
    }
    let key = format!("outside-change:{}:{target_root}", caller.turn_id);
    let data = json!({"text":format!("Changed by #{}: {what}", caller.thread_id)});
    if let Some(activity_id) = c
        .query_row(
    ...
    {
        c.execute(
            "UPDATE thread_activities SET data=?2,ts=?3 WHERE id=?1",
            params![
                activity_id,
                serde_json::to_string(
                    &json!({"text":format!("Changed by #{}: multiple changes", caller.thread_id)})
                )?,
                now
            ],
        )?;
        return Ok(Some(super::thread_activity::activity(c, activity_id)?));
    }
    c.execute("INSERT INTO thread_activities(thread_id,turn_id,kind,data,ts) VALUES(?1,NULL,'outside_change',?2,?3)", params![target_root,serde_json::to_string(&data)?,now])?;
    ...
    Ok(Some(super::thread_activity::activity(c, activity_id)?))
}
impl Storage {
    pub(crate) async fn mutate_scoped_thread(
        ...
        let mut result = match mutation {
            ...
            ThreadMutation::Create {
                ...
                crate::thread_rollups::refresh_from(&tx, thread.id)?;
            ...
            ThreadMutation::State { thread, headline } => {
                let id = thread_scope::resolve(&tx, caller.thread_id, thread)?;
                let changed = threads::set_headline(&tx, id, headline)?;
                let snapshots = changed
                    .into_iter()
                    .map(|id| threads::get(&tx, id))
                    .collect::<anyhow::Result<Vec<_>>>()?;
                json!({"thread_id":id,"thread":threads::get(&tx,id)?,"threads":snapshots})
            }
        ...
        if let Some(id) = result.get("thread_id").and_then(Value::as_u64) {
            ...
            let what = match mutation {
                ThreadMutation::Create { .. } => "created a Thread",
                ThreadMutation::State { .. } => "updated its headline",
                ...
                ThreadMutation::Grant { .. } | ThreadMutation::Revoke { .. } => "changed its reach",
                ThreadMutation::Cancel { .. } => "requested a stop",
                ThreadMutation::AddRelated { .. } | ThreadMutation::RemoveRelated { .. } => {
                    "changed Related"
                }
                ThreadMutation::Activity { .. } => "added an activity",
                ThreadMutation::Update { .. } => "updated it",
            };
            if let Some(activity) = outside_change(&tx, caller, id, what, &now)? {
                result["outside_activity"] = serde_json::to_value(activity)?;
            }
        }
