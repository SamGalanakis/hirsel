...
impl Contract {
            ...
            match object.get("type").and_then(Value::as_str) {
                ...
                Some("field") => {
                    ...
                    if contract.fields.len() >= MAX_FIELDS {
                        anyhow::bail!("Thread UI exceeds {MAX_FIELDS} fields");
                    }
                    if contract
                        .fields
                        .insert(
                        ...
                        .is_some()
                    {
                        anyhow::bail!("duplicate Thread field name `{name}`");
                    }
                }
                _ => {}
            }
    ...
    fn insert_action(
        ...
        if action.len() > MAX_ACTION_NAME_BYTES {
            anyhow::bail!("Thread action name exceeds {MAX_ACTION_NAME_BYTES} bytes");
        }
        if self.actions.len() >= MAX_ACTIONS {
            anyhow::bail!("Thread UI exceeds {MAX_ACTIONS} actions");
        }
        if self
            .actions
            ...
            .is_some()
        {
            anyhow::bail!("duplicate Thread action declaration `{action}`");
        }
        Ok(())
    }
}
...
fn validate_option_payload<'a>(
    ...
    let choice = payload_string(data, "choice", true)?
        .ok_or_else(|| anyhow::anyhow!("Thread action requires data.choice"))?;
    let label = options
        .get(choice)
        .ok_or_else(|| anyhow::anyhow!("unknown Thread action choice: {choice}"))?;
    if let Some(provided) = payload_string(data, "label", false)?
        && provided != label
    {
        anyhow::bail!("Thread action data.label does not match choice `{choice}`");
    }
    Ok(label)
}
...
fn validate_form_payload(
    ...
    if data.len() > MAX_FIELDS {
        anyhow::bail!("Thread action data exceeds {MAX_FIELDS} fields");
    }
    for key in data.keys() {
        if !fields.contains_key(key) {
            anyhow::bail!("unknown Thread action data field `{key}`");
        }
    }
    for (name, spec) in fields {
        let value = payload_string(data, name, spec.required)?;
        if spec.required && value.is_none_or(str::is_empty) {
            anyhow::bail!("Thread action requires non-empty data.{name}");
        }
    }
    Ok(())
}
...
fn payload_string<'a>(
    ...
    let Some(value) = data.get(key) else {
        if required {
            anyhow::bail!("Thread action requires data.{key}");
        }
        return Ok(None);
    };
    let value = value
        .as_str()
        .ok_or_else(|| anyhow::anyhow!("Thread action data.{key} must be a string"))?;
    if value.len() > MAX_FIELD_STRING_BYTES {
        anyhow::bail!("Thread action data.{key} exceeds {MAX_FIELD_STRING_BYTES} bytes");
    }
    Ok(Some(value))
}
...
fn validate_node(node: &Value, at: &str, depth: usize, nodes: &mut usize) -> anyhow::Result<()> {
    if depth > MAX_DEPTH {
        anyhow::bail!("Thread UI exceeds maximum nesting depth at {at}");
    }
    *nodes += 1;
    if *nodes > MAX_NODES {
        anyhow::bail!("Thread UI exceeds {MAX_NODES} components");
    }
    let object = node
        .as_object()
    ...
    match kind {
        ...
        other => anyhow::bail!("unknown Thread UI component `{other}` at {at}"),
    }
    Ok(())
}
    ...
    fn rejects_duplicate_actions_fields_and_unrendered_field_kinds() {
        ...
        assert!(
            validate(&duplicate_action)
                .unwrap_err()
                .to_string()
                .contains("duplicate Thread action")
        );
        ...
        assert!(
            validate(&duplicate_field)
                .unwrap_err()
                .to_string()
                .contains("duplicate Thread field")
        );
