...
/// The Info pane edits travel the ordinary thread_action path: fenced,
/// acknowledged by client_id, and broadcast as the updated Thread.
#[tokio::test]
async fn websocket_owner_edits_title_and_description_over_the_wire() {
    let dir = tempfile::tempdir().unwrap();
    let state = build_state(test_config(dir.path())).await.unwrap();
    let thread = state
        .storage
    ...
    // A stale revision loses; the Thread keeps its title.
    ws.send(Message::Text(
        serde_json::json!({
            "type": "thread_action",
            "client_id": "stale-title",
            "history_id": history_id,
            "thread_id": thread.id,
            "action": "set_title",
            "data": {"title": "Too late"},
            "expected_revision": thread.revision + 5
        })
        .to_string(),
    ))
    .await
    ...
    match read_error(&mut ws).await {
        HostToClient::Error { detail, client_id } => {
            assert!(detail.contains("thread changed"), "{detail}");
            assert_eq!(client_id.as_deref(), Some("stale-title"));
        }
        other => panic!("unexpected action error: {other:?}"),
    }
    ...
    .await
    .unwrap();
    // The broadcast and the acknowledgement both arrive, in either order.
    let mut renamed: Option<hirsel_proto::Thread> = None;
    let mut acknowledged: Option<String> = None;
    while renamed.is_none() || acknowledged.is_none() {
        match read_until(&mut ws, |response| {
            matches!(
                response,
                HostToClient::ThreadUpsert { .. } | HostToClient::ThreadActionApplied { .. }
            )
        })
        .await
        {
            HostToClient::ThreadUpsert { thread: upserted } if upserted.id == thread.id => {
                renamed = Some(upserted)
            }
            HostToClient::ThreadActionApplied { client_id, .. } => acknowledged = Some(client_id),
            _ => {}
        }
    }
    let renamed = renamed.unwrap();
    assert_eq!(renamed.title, "After");
    assert_eq!(renamed.revision, thread.revision + 1);
    assert_eq!(acknowledged.as_deref(), Some("rename"));
    let revision = renamed.revision;
    ...
    ws.send(Message::Text(
        serde_json::json!({
            ...
            "data": {"description": "What this Space is for."},
    ...
    .unwrap();
    match read_until(&mut ws, |response| {
        matches!(response, HostToClient::ThreadUpsert { thread: upserted } if upserted.id == thread.id && !upserted.description.is_empty())
    })
    .await
    {
        HostToClient::ThreadUpsert { thread: upserted } => {
            assert_eq!(upserted.description, "What this Space is for.");
            assert_eq!(upserted.title, "After");
            assert_eq!(upserted.execution, None);
        }
