...
export function conversationEntries(history: ThreadHistory): ConversationEntry[] {
  ...
  const unfinished = history.turns.filter(turn => turn.agent_message_id === null && (turn.owner_message_id !== null ? loaded.has(turn.owner_message_id) : inPage(turn.started_at ?? turn.accepted_at)));
  ...
  for (const turn of unfinished) {
    // Accepted work has no execution start until it actually runs.
    const time = instant(turn.started_at ?? turn.accepted_at);
