...
try {
      ...
      const injectRunningSummary = value => {
        ...
        if (value.id === thread.id && "running_turn" in value && "queued_turn_count" in value) {
          value.running_turn = {
            ...
            accepted_at: new Date().toISOString(), started_at: new Date().toISOString(), finished_at: null,
          };
  ...
  assert.equal(projectChat.artifactTitle, artifact.title, "opening the project chat reset the selected artifact");
  await page.locator(`[data-thread-row="${thread.id}"]`).click();
