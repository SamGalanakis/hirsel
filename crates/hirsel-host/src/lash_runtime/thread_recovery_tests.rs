...
#[tokio::test]
async fn admission_uses_the_profile_captured_before_a_kind_change() {
    ...
    let (project, _) = state
        ...
        .await
        .unwrap();
    accepted_request(&state, project.id, "project-capture").await;
    state
        .storage
        ...
        .await
        .unwrap();
    let project_runtime = runtime_lane(&state, Some(project.id)).await;
    let _project_pump = project_runtime.pump_lock.lock().await;
    project_runtime.admit_next_thread_request().await.unwrap();
    assert_eq!(
        *project_runtime.tool_profile.read().unwrap(),
    ...
    let (worker, _) = state
        ...
        .await
        .unwrap();
    accepted_request(&state, worker.id, "worker-capture").await;
    state
        .storage
        .set_addressed_thread_kind(
            &history,
        ...
        .await
        .unwrap();
    let worker_runtime = runtime_lane(&state, Some(worker.id)).await;
    let _worker_pump = worker_runtime.pump_lock.lock().await;
    worker_runtime.admit_next_thread_request().await.unwrap();
    assert_eq!(
        *worker_runtime.tool_profile.read().unwrap(),
