...
async fn native_worker_rlm_persists_code_tool_activity_and_final_message() {
    let (executor, storage, log, dir) = super::super::tests::test_event_executor().await;
...
finish("native read completed");
    ...
    let sink =
        NativeTimelineSink::new(thread_id, turn_id, executor.tools.clone(), broadcaster, log);
    let report = session
    ...
    let projection = native_terminal_projection(&output);
    assert_eq!(projection.state, ThreadTurnState::Completed);
    append_native_tool_completed_activities(
        &executor.tools,
