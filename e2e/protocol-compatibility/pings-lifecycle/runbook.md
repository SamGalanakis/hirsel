...
Prove the complete Ping lifecycle: required name and description fields, lifecycle-neutral Anchor replies and mentions, explicit Owner Event settlement, and Agent resolution of a moot Ping.

## Execute

```bash
export CARGO_TARGET_DIR=/workspace/.cargo-target-scaffold
e2e/protocol-compatibility/pings-lifecycle/run.sh
```
...
- An Owner message whose `ref` equals the Ping Anchor remains `status: "open"` and the scripted Agent acknowledges it.
- `POST /debug/event-action` with explicit `dismiss` changes the first Ping to done and broadcasts `event_upsert`.
