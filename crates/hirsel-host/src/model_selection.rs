...
pub fn validate_model_id_in_mode(
    ...
    if mode.is_free_text() {
        return validate_free_text(model_id);
    }
    let available = available_in_mode(mode, slot);
    let entry = available
        .iter()
        .find(|candidate| candidate.id == model_id)
        .ok_or_else(|| {
            anyhow!(
                "model `{model_id}` is not available on this provider; available models: {}",
    ...
    validate_in_mode(mode, slot, model_id, &entry.default_variant)
}
