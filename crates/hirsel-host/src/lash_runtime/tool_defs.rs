...
pub(super) fn hirsel_tool_definitions(
    ...
    let mut definitions = vec![
        ...
        tool_definition(
            "hirsel.threads_archive",
            "threads_archive",
            "Archive a Space or Task and its subtree: work is cancelled, it leaves the active tree, conversation and artifacts are kept, and the Owner or you can unarchive later. This is the only removal; there is no delete.",
            json!({"type":"object","additionalProperties":false,"properties":{"thread":thread_ref_schema()}}),
            thread_archive_result_schema(),
            ["threads"],
            "archive",
        ),
        tool_definition(
            "hirsel.threads_unarchive",
            "threads_unarchive",
            "Return an archived Space or Task and its subtree to the active tree. Cancelled work stays cancelled; nothing restarts on its own.",
            json!({"type":"object","additionalProperties":false,"properties":{"thread":thread_ref_schema()}}),
            thread_archive_result_schema(),
            ["threads"],
            "unarchive",
