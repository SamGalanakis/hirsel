...
export type ThreadClientMessage =
  ...
  | { type: "create_thread"; client_id: string; title: string; parent_thread_id: number | null }
  | { type: "open_thread"; client_id: string; thread_id: number; before_id: number | null }
  | { type: "send_thread_message"; client_id: string; thread_id: number; body: string; attachments: string[]; mentions: number[]; artifact_ids: number[]; mode: "send" | "next_turn" }
