...
/** A web link's actions occupy no inline space: right-click, long-press or
 * Shift+F10 on the link itself is the whole affordance. */
const openActions=(name:string)=>fireEvent.contextMenu(screen.getByRole('link',{name}));
describe("rich Markdown links",()=>{
 ...
 it("carries no inline control and leaves every navigating click to the browser",()=>{
  const {container}=mount(`[Review](${url})`);const link=screen.getByRole('link',{name:'Review'});const observed:boolean[]=[];
  // The chevron that used to follow every link is gone: a link is text.
  expect(container.querySelectorAll('button')).toHaveLength(0);expect(link.className).not.toContain('inline-flex');
  const observe=(event:Event)=>{observed.push(event.defaultPrevented);event.preventDefault();};document.addEventListener('click',observe);document.addEventListener('auxclick',observe);
  ...
  document.removeEventListener('click',observe);document.removeEventListener('auxclick',observe);expect(observed).toEqual([false,false,false,false,false,false]);expect(screen.queryByRole('menu')).toBeNull();
 });
 it("opens the same actions from the link's own context menu",async()=>{
  mount(`[Review](${url})`);const link=screen.getByRole('link',{name:'Review'});
  const event=new MouseEvent('contextmenu',{bubbles:true,cancelable:true});link.dispatchEvent(event);
  expect(event.defaultPrevented).toBe(true);expect(await screen.findByRole('menu')).toBeInTheDocument();
  expect((await screen.findAllByRole('menuitem')).map(item=>item.textContent)).toEqual(['Open in new tab','Copy link','Add to Related']);
 });
 it("copies exact destinations and surfaces clipboard rejection without writes",async()=>{
  mount(`[Review](${url})`);openActions('Review');fireEvent.click(await screen.findByRole('menuitem',{name:'Copy link'}));expect(clipboard).toHaveBeenCalledWith(url);
  clipboard.mockRejectedValueOnce(new Error('denied'));openActions('Review');fireEvent.click(await screen.findByRole('menuitem',{name:'Copy link'}));expect(await screen.findByRole('alert')).toHaveTextContent('Couldn’t copy');expect(frames).toEqual([]);
 });
 it("saves only on explicit action and reflects a duplicate from the authoritative snapshot",async()=>{
  mount(`[Review](${url})`);expect(frames).toEqual([]);openActions('Review');fireEvent.click(await screen.findByRole('menuitem',{name:'Add to Related'}));
  ...
  await waitFor(()=>expect(screen.queryByRole('alert')).toBeNull());openActions('Review');expect(await screen.findByRole('menuitem',{name:'Already in Related'})).toBeDisabled();expect(frames).toHaveLength(1);
 });
 it("rejects an action from a rendered old-history message",async()=>{
  mount(`[Review](${url})`);flush(()=>setHistoryId('replacement'));openActions('Review');fireEvent.click(await screen.findByRole('menuitem',{name:'Add to Related'}));expect(await screen.findByRole('alert')).toHaveTextContent('History changed');expect(frames).toEqual([]);
 });
});
