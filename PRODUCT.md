...
Conversation is the resting surface. Its Artifacts view lists the results referenced in that Thread; the global Artifacts inventory links each result back to its conversations. Opening or expanding a result never changes the Thread addressed by the composer. Work summaries show the current action or final outcome beside the exact response. One expansion reveals ordered steps; raw event data remains under Diagnostics. Failed and stopped runs remain visible without expansion, and plain replies have no empty work control. Thread instruments retain their existing action validation and revision checks; they are distinct from artifacts.
