...
The component vocabulary is intentionally closed. It supports text, layout, statuses, options and forms without executing generated code. Plugin view events remain separate current capabilities; host-authored Views use the Canvas surface. There is no Event/Ping inventory, Event action alias, or side-session placement.
