...
import { Pin, GitBranch, Archive, Check, Clock, Copy, MoreHorizontal, RotateCcw, Square, SquarePen } from "../components/ui/icons";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "../components/ui/dropdown-menu";
import { threadActions, type ThreadActionIcon } from "./actions";
import type { Thread } from "./types";
const control = "size-11 shrink-0 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring";
const icons = { icon: SquarePen, pin: Pin, child: GitBranch, settle: Check, reopen: RotateCcw, read: Check, snooze: Clock, archive: Archive, copy: Copy, stop: Square };
...
export function ThreadActions(props: { thread: Thread; quick?: boolean; now?: number }) {
  ...
  return <div class="flex shrink-0 items-start">
    ...
    <DropdownMenu>
      <DropdownMenuTrigger data-thread-actions={props.thread.id} class={`${control} inline-flex`} aria-label={props.quick ? `Actions for ${props.thread.title}` : "Thread actions"} title="Thread actions"><MoreHorizontal class="size-4" /></DropdownMenuTrigger>
