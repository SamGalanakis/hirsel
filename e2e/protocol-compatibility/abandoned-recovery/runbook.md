...
The runner uses `HIRSEL_AGENT=lash HIRSEL_PROVIDER=codex HIRSEL_DRIVER=fake` with a complete standalone change in a tiny `/tmp` Git repo and a long fake fixture, SIGKILLs only the host PID it started, then reboots on the same data dir.
...
- After reboot, `/debug/processes` surfaces the original process id as `state: "abandoned"` and never returns that id to `running`.
- The Agent is woken and emits Chat or Ping output that references the orphaned/abandoned work.

The Agent may choose to spawn a replacement with a new process id after the abandonment wake. That is the judgment ADR-0004 requires, not mechanical resurrection of the abandoned Runtime Process.
