...
// that into an revision-checked `thread_action` frame. optionList emits
...
const useInstrumentEmit = () => useContext(InstrumentEmitContext);
...
function HeadingNode(node: Node): JSX.Element {
  // The Thread identity immediately above the generated instrument is its h2.
...
const InstrumentNode = createNodeDispatch(REGISTRY, (type) => (
    ...
    <Show
      ...
      fallback={
        <div class="flex flex-col gap-2.5">
          <For each={props.nodes}>{(n) => <InstrumentNode node={n} />}</For>
        </div>
      }
    >
...
export interface ThreadInstrumentProps {
  /** The constrained JSON-UI card body — a root node or an array of nodes. */
  ui: ViewSpec | ViewSpec[] | undefined;
  /** Post an interaction back (the caller turns it into an `thread_action`). */
...
export function ThreadInstrument(props: ThreadInstrumentProps): JSX.Element {
  const [values, setValues] = createSignal<Record<string, unknown>>({});
  const emit: InstrumentEmit = {
  ...
  return (
    ...
    >
      <InstrumentEmitContext value={emit}>
        <CardMetaContext value={{ get eyebrowAge() { return props.eyebrowAge; } }}>
          <NodeList nodes={instrumentNodes(props.ui)} rhythm />
        </CardMetaContext>
      </InstrumentEmitContext>
    </Errored>
  );
}

function instrumentNodes(ui: ViewSpec | ViewSpec[] | undefined): ViewSpec[] {
  if (Array.isArray(ui)) return ui.filter(node => node && typeof node.type === "string");
  if (!ui || typeof ui.type !== "string") return [];
  if (ui.type === "card" && Array.isArray(ui.children)) return ui.children.filter(node => node && typeof node.type === "string");
  return [ui];
}
