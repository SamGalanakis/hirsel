...
/** The composer's Thread-ref picker. Detection and insertion are pure (see
 * lib/thread-ref.ts); this hook holds the reactive open/query/active-row state,
 ...
 * lands on the picker's rung of the ladder instead of clearing Thread focus
 * behind it. */
export function createThreadRefPicker(opts: {
  ...
  const candidates = createMemo<RefTarget[]>(() => {
    ...
    return filterThreadCandidates(opts.threads(), q.query);
  });
   ...
   * query that matches no Thread closes rather than showing an empty box — an
  ...
  function sync(typed = false): void {
    ...
    if (q === null || filterThreadCandidates(opts.threads(), q.query).length === 0) {
      close();
      return;
    }
    setQuery(q);
  ...
  /** Write the chosen Thread's ref at the caret, then close and restore the caret
   * after the token. */
  function accept(thread: RefTarget): void {
    const el = opts.getEl();
    const q = query();
    if (!el || !q) return;
    const caret = el.selectionStart ?? opts.value().length;
    const next = insertThreadRef(opts.value(), q, caret, thread.id);
