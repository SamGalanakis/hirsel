...
fn showcase_edits_reject_reused_thread_identity_after_history_change() {
    ...
    let thread: hirsel_proto::Thread = serde_json::from_value(json!({
        "id":5,"parent_thread_id":null,"pinned_at":null,"title":"Same ID","icon":"🐙",
    ...
    assert!(!client.update_thread_showcase("history-a".into(), 5, Some(42), 7));
    assert!(!client.update_thread_showcase("history-b".into(), 5, Some(42), 6));
    assert!(!client.update_thread_showcase("history-b".into(), 99, Some(42), 7));
    client.inner.write_store().connection = ConnectionState::Offline;
    assert!(!client.update_thread_showcase("history-b".into(), 5, Some(42), 7));
    assert!(client.inner.pending_frames.lock().unwrap().is_empty());
    client.inner.write_store().connection = ConnectionState::Online;
    assert!(client.update_thread_showcase("history-b".into(), 5, None, 7));
