...
helpers from `e2e/protocol-compatibility/channel-discipline/runbook.md`, plus:
...
Then Sam answers through the generated Task action and the Task settles through `event_action`:
...
- Gate 2: no Chat nag re-asking the open question over the window; interrupt count stays at 1;
  generated answer settles it through the explicit Event action path.
...
Per `e2e/protocol-compatibility/RULES.md`: record Ping ids, `requires_response` counts, the resolve `ping_upsert`, and the
