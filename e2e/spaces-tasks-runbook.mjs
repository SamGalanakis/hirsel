...
function storeSnapshot() {
  return {
    ...
    turns: sqliteJson("SELECT id,thread_id,requester_thread_id,owner_message_id,agent_message_id,state,accepted_at,started_at,finished_at FROM thread_turns ORDER BY id"),
...
try {
  ...
  sqliteRun(`
    ...
    INSERT INTO thread_turns(thread_id,state,accepted_at,started_at,finished_at)
      VALUES(${rootSpace.id},'completed','2026-09-10T11:00:00Z','2026-09-10T11:00:00Z','2026-09-10T11:01:00Z');
    COMMIT;
  `);
  ...
  sqliteRun(`
    PRAGMA foreign_keys=ON;
    INSERT INTO thread_turns(thread_id,state,accepted_at)
