...
const createControl = "inline-flex min-h-11 min-w-0 items-center justify-center gap-2 whitespace-nowrap rounded-lg bg-muted px-2 text-sm text-foreground transition-colors hover:bg-muted/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-40";
export function ThreadNavigation(props: { open: boolean; modal: boolean; intent: ThreadNavigationIntent | null; onClose: () => void; onSelect: (id: number) => void }) {
  ...
  return <dialog ref={node => { dialog = node; }} id="thread-navigation" role={props.modal ? "dialog" : "complementary"} aria-modal={props.modal ? "true" : undefined} aria-label="Spaces and Tasks" data-slot="thread-drawer"
    ...
    <header class="mb-4 flex flex-col gap-2"><div data-slot="thread-drawer-identity" class="min-w-0"><h2 class="text-base font-semibold">Spaces & Tasks</h2><p data-slot="thread-filter-caption" class="text-xs capitalize text-muted-foreground">{section() === "settled" ? "done" : section()}</p></div><div data-slot="thread-drawer-actions" class="flex justify-end"><DropdownMenu><DropdownMenuTrigger data-thread-filter class={`${control} ${section() !== "active" ? "bg-muted text-foreground" : ""}`} aria-label={`Filter work: ${section() === "settled" ? "done" : section()}`} title={`Filter work: ${section() === "settled" ? "done" : section()}`}><Funnel class="size-4" /></DropdownMenuTrigger><DropdownMenuContent><For each={["active", "settled", "snoozed", "archived"] as const}>{name => <DropdownMenuItem role="menuitemradio" aria-checked={section() === name ? "true" : "false"} class="min-h-11 capitalize" onSelect={() => setSection(name)}><Check class={section() === name ? "size-4" : "size-4 invisible"} />{name === "settled" ? "done" : name}</DropdownMenuItem>}</For></DropdownMenuContent></DropdownMenu><button class={control} aria-label="Search spaces and tasks" title="Search spaces and tasks" onClick={openThreadSearch}><Search class="size-4" /></button><button class={control} aria-label="Close Spaces and Tasks" title="Close Spaces and Tasks" onClick={props.onClose}><X class="size-5" /></button></div></header>
    ...
    <form onSubmit={event => void add(event)} class="mb-3 space-y-2"><input aria-label="New space or task title" placeholder={parentId() === null ? "Name this Space or Task…" : parent()?.kind === "task" ? "Name this Task…" : "Name this child…"} value={title()} onInput={event => setTitle(event.currentTarget.value)} class="h-11 w-full min-w-0 rounded-lg border border-border bg-transparent px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" /><div data-slot="thread-create-actions" class="grid grid-cols-2 gap-2"><Show when={canCreateSpace()}><button type="submit" name="kind" value="space" class={createControl} disabled={!title().trim() || creating() || state.connection !== "connected"}><Plus class="size-4 shrink-0" /><span>New Space</span></button></Show><button type="submit" name="kind" value="task" class={`${createControl} ${canCreateSpace() ? "" : "col-span-2"}`} disabled={!title().trim() || creating() || state.connection !== "connected"}><Plus class="size-4 shrink-0" /><span>New Task</span></button></div></form>
          ...
          return <>
            <li data-thread-entry={row().key} data-context={row().context ? "true" : undefined} class={`relative flex shrink-0 items-start rounded-lg ${threadState.focusedId === thread().id ? "bg-muted/60" : "hover:bg-muted/30"}`} style={{ "margin-left": `clamp(0px, calc(100% - 13.5rem), ${Math.min(row().depth, 4) * 12}px)` }}>
              ...
              <button onContextMenu={event => { event.preventDefault(); event.currentTarget.parentElement?.querySelector<HTMLButtonElement>('[data-thread-actions]')?.click(); }} onKeyDown={event => { move(event); if (event.key === "ContextMenu" || (event.key === "F10" && event.shiftKey)) { event.preventDefault(); event.currentTarget.parentElement?.querySelector<HTMLButtonElement>('[data-thread-actions]')?.click(); } }} data-thread-row={thread().id} data-row-key={row().key} class="block min-h-11 min-w-0 flex-1 rounded-lg px-2 py-2 text-left text-sm text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-ring" aria-current={threadState.focusedId === thread().id ? "page" : undefined} title={threadPath(threadState.threads, thread().id)} onClick={() => props.onSelect(thread().id)}>
                <span class="block min-h-8 pl-11 pr-11 font-medium text-foreground min-[420px]:pr-[5.5rem]"><span data-slot="thread-row-title" class="block break-words">{thread().title}</span><span class="mt-0.5 flex items-center gap-1.5 text-xs font-normal capitalize text-muted-foreground"><ThreadAvatar thread={thread()} /><span>{thread().kind}</span><Show when={thread().parent_thread_id === null && thread().pinned_at}><Pin class="size-3 shrink-0" /><span class="sr-only">Pinned</span></Show></span></span>
