...
from `e2e/protocol-compatibility/lib/runbook-lib.sh`. All debug requests must carry
`Authorization: Bearer $HIRSEL_TOKEN`. Add:

```bash
# In the controlling shell, after setting REPO, HIRSEL_TOKEN, and BASE:
source "$REPO/e2e/protocol-compatibility/lib/runbook-lib.sh"
...
host-timer return, restart survival, unsnooze, the sweep — is `e2e/protocol-compatibility/event-snooze-sweep`.)
...
Per `e2e/protocol-compatibility/RULES.md`: record event ids, the choose payload, the anchor-refed reply's Chat id, the taste
