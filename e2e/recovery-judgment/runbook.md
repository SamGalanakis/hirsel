...
This is the judgment layer on top of `e2e/abandoned-recovery` (which covers the no-respawn safety
...
Per `e2e/RULES.md`: record the KEEP/DROP process ids, the pre-crash `subagents_interrupt`, the
post-reboot process list, and the nudge turn. If abandoned-process visibility is empty after reboot,
note it as the known open finding (cross-ref `e2e/abandoned-recovery`) — it does not fail this
