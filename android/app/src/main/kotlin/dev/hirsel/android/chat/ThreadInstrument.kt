...
@Composable
private fun InstrumentNode(node: Any?, fields: MutableMap<String, String>, enabled: Boolean, submit: (String, JSONObject) -> Unit) {
    val colors = LocalHirselColors.current
    when (node) {
        is JSONArray -> repeat(node.length()) { InstrumentNode(node.opt(it), fields, enabled, submit) }
        is JSONObject -> when (node.optString("type")) {
            "card", "inset" -> Column { InstrumentNode(node.optJSONArray("children"), fields, enabled, submit) }
            ...
            "submit" -> Button(onClick = { submit(node.optString("action", "submit"), JSONObject(fields.toMap())) }, enabled = enabled) { Text(node.optString("label")) }
            "optionList" -> node.optJSONArray("options")?.let { options -> repeat(options.length()) { i -> options.optJSONObject(i)?.let { option ->
