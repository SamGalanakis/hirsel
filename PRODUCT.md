...
> Space-chat amendment, 2026-09-21: [ADR 0025](docs/adr/0025-space-chats-essentials.md) supersedes the earlier “no default recipient” landing rule. Route-free entry restores the last active Space, then the first active top-level Space, then creates Home through the ordinary create path; explicit bad links still never redirect.
...
The composer sends to an explicit recipient and Messages belong to exactly one Thread. Every Space has a Space chat in its ordinary Thread conversation. Route-free entry restores the last active top-level Space for this history, otherwise opens the first active top-level Space by ID, otherwise creates and opens the ordinary top-level Space named Home through the normal create path. Home has an ordinary positive ID and no automatic root grant. Every actual ID, including a retained ID 0, remains an ordinary Thread. Explicit invalid, malformed, missing and old-history links remain unaddressed instead of falling back to Home.

The composer always labels its one recipient and says plainly when that recipient is a Task's worker. “Talk about this” opens a Task's owning Space chat and inserts a `#<id>` reference into the draft; the agent reads the Task through ordinary reach, and an out-of-reach reference produces the existing typed refusal. “Step in” addresses the Task's own worker and visibly offers “Send after current turn” while work is running; Stop is separate. Drafts are keyed by history and the actual recipient.
...
- **Headline:** each Thread has a normalized nonempty own headline of at most 12 words. A worker updates it with `threads.state`; the previous displayed headline and an independent headline revision support the board's changed-since-you-looked projection.
...
- **Outside changes:** when a Thread in one top-level Space changes a Thread in another, the affected Space chat shows one ordinary, per-acting-turn-coalesced `Changed by #<id>: <what>` activity. It does not wake the Space, inject context or grant reach; an agent rereads current state when needed.
