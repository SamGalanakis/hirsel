...
- **Danger** (`oklch(0.704 0.191 22.216)`, `--status-danger` = `--destructive`): A red reserved for the genuine "blocked on you" escalation (the one Inbox badge that carries an open requires-response Ping — the phone Tray shelf badge, and its desktop equivalent the nav rail's Inbox badge) and destructive confirms (Discard). It is the loudest thing in the system and appears least. The standing Pings rail's own header count is deliberately muted, never red, so the interrupt is never doubled on desktop.
- **Idle** (`oklch(0.65 0 0)`, `--status-idle`): Pure neutral gray for dormant state.

### Named Rules
**The One Escalation Rule.** Exactly one color means "genuinely blocked on you": danger red, and only on the single Inbox badge for an open requires-response Ping — the phone Tray shelf badge, or on desktop the nav rail's Inbox badge (the desktop equivalent). The standing Pings rail header count is muted, the nav's Processes badge uses a status-active *tint* chip (not a solid disc), and the nav's active item uses the muted fill — so nothing else competes with the one red. Indigo means "interactive / worth a look," amber means "transient hiccup," but red is the single interrupt. Do not multiply reds. If two things on screen are shouting, one of them is wrong.
...
### Navigation
- **Phone / narrow:** the phone-first single column stays — Chat is the whole app. The only chrome is a thin `border-b` top bar (wordmark left; Processes icon + connection pill right), with shelves, sheets, and overlays doing the rest. No nav rail.
- **Desktop (`rail`, ≥1100px):** a persistent 3-pane shell. A left **nav rail** (wordmark; Chat, Inbox with its unread badge, Processes with a status-active *tint* chip, and a Settings gear; the connection pill pinned at the foot above an always-present `border-t`) ∣ the center **chat pane** ∣ an always-present right **context pane** (the Pings rail resting state / Side Chat / Processes + Settings inspectors). The three panes share one **top datum**: the nav-rail brand block, a slim center-pane header, and every pane title sit on the same `h-12` baseline with a single continuous `border-b` hairline running across all three. The center header's left carries a calm **agent-status indicator** (a status dot + label — pulsing status-active while the agent is thinking, quiet muted "Agent · idle" otherwise), giving the north-star question "what is my agent doing" a persistent desktop home; its right is reserved. The center **chat pane** keeps the prose measure (centered in the pane) for the transcript, and the **composer bar** spans the pane full-width (rail hairline → context hairline) while its input row re-centers at that same measure — no naked canvas beside a lonely column. A sparse transcript bottom-anchors (grows up from the composer). The width is used: the frame fills to a ~1600px cap and only then centers. The nav's active item uses the **muted fill** (never indigo), so the One-Escalation / indigo-is-rare rules still hold. Settings is a clearly-marked, left-aligned placeholder for a later pass.
