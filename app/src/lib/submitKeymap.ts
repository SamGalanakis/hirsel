...
// stop/cancel) around this call — those differ per surface — but
...
export interface SubmitKeymapHandlers {
   ...
   * Composer sends; Cmd/Ctrl+Enter sends and adding Shift queues). */
  onSend: () => void;
  /** Explicit next-turn submission; ordinary Tab always keeps native focus. */
  onQueue?: () => void;
 ...
 *   • Cmd/Ctrl+Enter sends; Cmd/Ctrl+Shift+Enter queues when supported.
 ...
 * it returns false the caller's own keys still run. */
