...
const helloThreads = async (threads: ReturnType<typeof makeThread>[]) => {
  flush(() => handleThreadMessage({ type: "hello_ok", history_id: "ab123456-1234-5678-9abc-123456789abc", threads, processes: [], views: [], host_version: "test", model: null, subagent_models: null, prompts: null, providers: null }));
...
describe("explicit Thread selection", () => {
  it("creates Home only for an empty forest and otherwise opens the first Space", async () => {
    await hello([]); expect(threadState.focusedId).toBeNull(); expect(frames).toContainEqual(expect.objectContaining({type:"create_thread", title:"Home", kind:"space", parent_thread_id:null}));
    frames.length = 0;
    await hello([0,1,2]); expect(threadState.focusedId).toBe(0); expect(frames).toContainEqual(expect.objectContaining({type:"open_thread", thread_id:0}));
  });
  ...
  it("addresses a nested Space as a Space chat instead of pairing it as a worker", async () => {
    ...
    await helloThreads([home, nested]);
  ...
  it("pairs a Task with its nearest containing Space chat", async () => {
    const home = makeThread(1, { title: "Home", kind: "space", parent_thread_id: null });
    const nested = makeThread(2, { title: "Planning", kind: "space", parent_thread_id: home.id });
    const task = makeThread(3, { title: "Draft", kind: "task", parent_thread_id: nested.id });
    await helloThreads([home, nested, task]);
  ...
  it("prioritizes an explicit ordinary zero route over saved and focused IDs", async () => {
  ...
  it("restores only a valid selection from this history", async () => {
    ...
    await hello([1,2]); expect(threadState.focusedId).toBe(1); expect(frames).toContainEqual(expect.objectContaining({type:"open_thread",thread_id:1}));
    history.replaceState(null, "", "/");
    localStorage.setItem("hirsel.last-space.ab123456-1234-5678-9abc-123456789abc", "2");
    await hello([1,2]); expect(threadState.focusedId).toBe(2); expect(location.pathname).toBe("/t/2");
  });
  it("never redirects a missing explicit destination to saved or pinned work", async () => {
    ...
    await hello([1,2]); expect(threadState.focusedId).toBeNull(); expect(threadState.linkError).toContain("unavailable"); expect(location.pathname).toBe("/t/99");
  });
  it("restores the last Space after returning to the route-free entry", async () => {
    await hello([1]); flush(() => focusThread(1)); flush(() => focusThread(null));
    frames.length = 0;
    await hello([1]); expect(threadState.focusedId).toBe(1); expect(frames).toContainEqual(expect.objectContaining({type:"open_thread",thread_id:1}));
    flush(() => setThreadState(draft => { draft.focusedId = 1; }));
    flush(() => resetThreads()); expect(threadState.focusedId).toBeNull();
  });
  it("preserves route intent across reset and refuses reused IDs in a new history", async () => {
    history.replaceState(null,"", "/t/1?history=ab123456-1234-5678-9abc-123456789abd");
    await hello([1]); expect(threadState.focusedId).toBeNull(); expect(threadState.linkError).toContain("another Hirsel history"); expect(frames).toEqual([]);
    flush(resetThreads); expect(location.search).toContain("123456789abd");
    await hello([1]); expect(threadState.focusedId).toBeNull(); expect(frames).toEqual([]);
  });
  it("resolves an unqualified route against the connected history and still refuses unknown IDs", async()=>{
    localStorage.setItem("hirsel.last-space.ab123456-1234-5678-9abc-123456789abc","1");
    history.replaceState(null,"","/t/1"); await hello([1]);
  ...
  it("refuses an unqualified route to a Thread this history does not have", async()=>{
    history.replaceState(null,"","/t/9"); await hello([1]);
    expect(threadState.focusedId).toBeNull(); expect(threadState.linkError).toContain("#9 is unavailable"); expect(frames).toEqual([]);
  });
  it("waits for a fresh hello before navigating cached IDs or sending", async()=>{
    await hello([1,2]); flush(()=>focusThread(1)); flush(disconnectThreads); frames.length=0;
    ...
    await hello([1,2]); expect(threadState.focusedId).toBe(2); expect(frames).toContainEqual(expect.objectContaining({type:"open_thread",thread_id:2}));
  });
  it("validates the first hello against its payload while the history signal is still pending", async()=>{
    ...
    flush(() => handleThreadMessage({type:"hello_ok",history_id:"ab123456-1234-5678-9abc-123456789abc",threads:[makeThread(2)],processes:[],views:[],host_version:"test",model:null,subagent_models:null,prompts:null,providers:null}));
    await Promise.resolve();
    await Promise.resolve();
  ...
  it("does not restore an archived selection from an authoritative snapshot", async () => {
    localStorage.setItem("hirsel.last-space.ab123456-1234-5678-9abc-123456789abc", "2");
    await helloThreads([makeThread(1), makeThread(2, { archived_at: "2026-09-10T10:00:00Z" })]);
  ...
  it("clears a previously focused Thread when a route-free snapshot archives it", async () => {
    await hello([2]);
    flush(() => focusThread(2));
    history.replaceState(null, "", "/");
    await helloThreads([makeThread(2, { archived_at: "2026-09-10T10:00:00Z", revision: 2 })]);
  ...
  it("keeps an explicit archived Thread route selectable", async () => {
    history.replaceState(null, "", "/t/2?history=ab123456-1234-5678-9abc-123456789abc");
    await helloThreads([makeThread(2, { archived_at: "2026-09-10T10:00:00Z" })]);
    expect(threadState.focusedId).toBe(2);
    expect(location.pathname).toBe("/t/2");
    expect(frames).toContainEqual(expect.objectContaining({ type: "open_thread", thread_id: 2 }));
  });
