...
describe("dev mock Thread contract", () => {
  it("isolates tokens while replaying owned histories and explicit lifecycle", async () => {
    ...
    expect(renamed.title).toBe("Renamed runbook");
    await expectActionError(connection.ws, addressed({ type: "thread_action", client_id: "action-title-blank", thread_id: created.id, action: "set_title", data: { title: "   " }, expected_revision: renamed.revision }), "title is required");
    const described = (await request({ type: "thread_action", thread_id: created.id, action: "set_description", data: { description: "## Scope" }, expected_revision: renamed.revision }, "thread_upsert")).thread as Thread;
    expect(described.description).toBe("## Scope");
    expect(described.execution).toBeNull();
    const routed = (await request({ type: "thread_action", thread_id: created.id, action: "set_execution", data: { execution: { kind: "cli", agent: "claude", model: "claude-opus-4-7", variant: "high" } }, expected_revision: described.revision }, "thread_upsert")).thread as Thread;
    expect(routed.execution).toEqual({ kind: "cli", agent: "claude", model: "claude-opus-4-7", variant: "high" });
    await expectActionError(connection.ws, addressed({ type: "thread_action", client_id: "action-execution-unknown", thread_id: created.id, action: "set_execution", data: { execution: { kind: "cli", agent: "nonesuch", model: "m", variant: "default" } }, expected_revision: routed.revision }), "Unknown agent");
    await expectActionError(connection.ws, addressed({ type: "thread_action", client_id: "action-execution-stale", thread_id: created.id, action: "set_execution", data: { execution: null }, expected_revision: described.revision }), "changed");
    const reset = (await request({ type: "thread_action", thread_id: created.id, action: "set_execution", data: { execution: null }, expected_revision: routed.revision }, "thread_upsert")).thread as Thread;
    expect(reset.execution).toBeNull();
    ...
    const space = (await request({ type: "create_thread", client_id: "space", title: "Space", kind: "space", parent_thread_id: null }, "thread_created")).thread as Thread;
    const area = (await request({ type: "create_thread", client_id: "area", title: "Area", kind: "space", parent_thread_id: space.id }, "thread_created")).thread as Thread;
    await expectActionError(connection.ws, addressed({ type: "thread_action", client_id: "space-parent-task", thread_id: space.id, action: "set_kind", data: { kind: "task" }, expected_revision: space.revision }), "child Spaces");
    ...
    expect(convertedArea).toMatchObject({ id: area.id, kind: "task", parent_thread_id: space.id });
    const convertedSpace = (await request({ type: "thread_action", thread_id: space.id, action: "set_kind", data: { kind: "task" }, expected_revision: space.revision }, "thread_upsert")).thread as Thread;
    expect(convertedSpace.kind).toBe("task");
    const sameKindAck = waitForFrame(connection.ws, frame => frame.type === "thread_action_applied" && frame.client_id === "same-kind");
    connection.ws.send(JSON.stringify(addressed({ type: "thread_action", client_id: "same-kind", thread_id: convertedSpace.id, action: "set_kind", data: { kind: "task" }, expected_revision: convertedSpace.revision })));
    expect(await sameKindAck).toMatchObject({ thread_id: convertedSpace.id });
