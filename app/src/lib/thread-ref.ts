...
export interface RefTarget { id: number; title: string; icon?: string | null; attention?: "quiet" | "needs_owner"; settled_at?: string | null; }
 ...
 * picker also filters by Thread name, so a partially-typed name (`#depl`) is a
...
/** The one spelling of a Thread ref. */
export function formatThreadRef(id: number): string {
  return `${THREAD_REF_TRIGGER}${id}`;
}

/** Parse a bare ref (`#12`, or a lone `12`) to its Thread id, or null. */
export function parseThreadRef(token: string): number | null {
...
export function detectRefQuery(text: string, caret: number): RefQuery | null {
  ...
  if (i === 0 || text[i - 1] !== THREAD_REF_TRIGGER) return null;
  const at = i - 1;
...
/** How well one Thread answers a query — lower is a better answer. `null` means
 * it does not answer it at all. */
function rank(thread: RefTarget, query: string): number | null {
  if (query.length === 0) return 4;
  const id = String(thread.id);
  if (id === query) return 0;
  if (id.startsWith(query)) return 1;
  const name = normalize(thread.title);
...
/** Threads answering `query`, best answer first; ties go newest-first (higher id).
 * An empty query lists the whole field, capped at `limit`. */
export function filterThreadCandidates(
  threads: RefTarget[],
  query: string,
  limit = 6,
): RefTarget[] {
  return threads
    .map((thread) => ({ thread, rank: rank(thread, query) }))
    .filter((row): row is { thread: RefTarget; rank: number } => row.rank !== null)
    .sort((a, b) => (a.rank !== b.rank ? a.rank - b.rank : b.thread.id - a.thread.id))
    .slice(0, limit)
    .map((row) => row.thread);
}

/** Replace the in-progress `#query` (from its `#` up to `caret`) with the chosen
 * Thread's ref plus a trailing space, returning the new text and caret. */
export function insertThreadRef(
  ...
  const token = spaced ? formatThreadRef(id) : `${formatThreadRef(id)} `;
  return {
...
/** One run of rendered text: either plain prose, or a resolved Thread ref. */
export interface RefSpan {
  text: string;
  /** The cited Thread, or null when this span is ordinary text. */
  threadId: number | null;
}

/** Split a text run into plain and ref spans for rendering. A ref naming no
 * live Thread stays in a plain span, so an unknown or archived citation degrades
 * to exactly the characters the author typed. */
export function splitThreadRefs(text: string, isKnown: (id: number) => boolean): RefSpan[] {
  const spans: RefSpan[] = [];
  let cursor = 0;
  for (const ref of scanRefs(text)) {
    if (!isKnown(ref.id)) continue;
    if (ref.index > cursor) spans.push({ text: text.slice(cursor, ref.index), threadId: null });
    spans.push({ text: ref.token, threadId: ref.id });
    cursor = ref.index + ref.token.length;
  }
  if (cursor < text.length) spans.push({ text: text.slice(cursor), threadId: null });
  return spans;
}
