...
3089) and the runner sources the shared `e2e/lib/runbook-lib.sh` helpers (`post_json` / `wait_jq` /
...
(task #20). Cross-ref `e2e/abandoned-recovery` for the sibling pattern of a documented known-open
...
Per `e2e/RULES.md`: record the Chat ids for the fact and recall turns and the `turn_event` that
