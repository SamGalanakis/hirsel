...
This layout scenario makes no model calls. It copies the saved `artifact-presentation` state into a new evidence directory, boots that copy with the scripted/fake Host on an unused non-3076 port, selects its real stored SVG artifact through the UI, and checks 2048, 1440, 1024, 768, 390, and 320 CSS-pixel viewports plus live resize, focus, persistence, and return to the Space chat. It also adds four ancestors to the copied fixture, supplies a structurally valid running-plus-queued Thread summary through the isolated WebSocket, and disconnects the browser so the real atomic “Last known: Working · 1 queued” rendering is checked at 1440 and 320px. The check covers the Thread row and inventory bounds, horizontal scroll position, and reachable non-overlapping row actions.
