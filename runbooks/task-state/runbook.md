...
6. Reload. Verify the authenticated `hello_ok`, `open_thread`, rendered state card and SQLite rows agree on state, artifact IDs, revisions and change causes.

## Evidence and verdict

Retain sanitized screenshots before and after reload, authenticated frames, `open_thread` snapshots, and exact rows from `thread_state`, `thread_state_artifacts`, `thread_state_changes`, `artifacts` and `thread_effect_receipts`. The `threads.state` operation must have one `edited` receipt. Record `OBJECTIVE_PASS` only for deterministic predicates; a reviewing Agent separately judges that the headline is useful, findings are material, rollup prose is factual, and no parent was automatically completed.
