...
- **Record:** if it is a settled fact, write one factual update with `fork.record_info`, one digest with `fork.record_summary`, as coordinator activity on an existing Thread. State the outcome in the Owner's terms; never paste raw logs.
