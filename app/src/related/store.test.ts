...
describe("Related request identity and snapshots", () => {
  ...
  it("uses link snapshot revision independently of newer Thread metadata", () => {
    ...
    flush(() => handleRelatedMessage({type:"thread_opened",client_id:"read",detail:{thread:makeThread(1,{revision:4}),related_items:[],grants:[],brief:{text:"",artifact_ids:[]},messages:[],turns:[],turn_timelines:[],activities:[],has_more:false}}));
  ...
  it("rejects old-history actions, broadcasts and delayed detail responses after reused IDs", async () => {
    ...
    receive(30,[link]);
    flush(()=>handleRelatedMessage({type:"thread_opened",client_id:"old-read",detail:{thread:makeThread(1,{revision:30}),related_items:[link],grants:[],brief:{text:"",artifact_ids:[]},messages:[],turns:[],turn_timelines:[],activities:[],has_more:false}}));
  ...
  it("correlates errors and retries a load without losing existing links", async () => {
    ...
    flush(()=>handleRelatedMessage({type:"thread_opened",client_id:id,detail:{thread:makeThread(1,{revision:3}),related_items:[link],grants:[],brief:{text:"",artifact_ids:[]},messages:[],turns:[],turn_timelines:[],activities:[],has_more:false}}));await retry;flush();expect(relatedState.lists[1].error).toBeNull();
  });
  it("captures the authoritative hello history for a read before the history signal commits",()=>{
    ...
    flush();
    flush(()=>handleRelatedMessage({type:"thread_opened",client_id:"first-read",detail:{thread:makeThread(1,{revision:2}),related_items:[link],grants:[],brief:{text:"",artifact_ids:[]},messages:[],turns:[],turn_timelines:[],activities:[],has_more:false}}));
