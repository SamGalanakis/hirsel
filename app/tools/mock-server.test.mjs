...
test("Thread mock preserves identity, owned history and lifecycle across reconnect", async () => {
  ...
  try {
    ...
    await once(a.ws, "open"); a.send({ type: "hello", token: "a" });
    assert.equal((await a.next("hello_ok")).threads.length, 1);
    ...
    a.send({ type: "create_thread", client_id: "create", title: "Buy groceries" });
    ...
    a.send({ type: "send_thread_message", client_id: "message", thread_id: thread.id, body: "Milk", attachments: [], mentions: [0] });
    ...
    a.send({ type: "send_thread_message", client_id: "message", thread_id: thread.id, body: "Milk" });
    ...
    await once(b.ws, "open"); b.send({ type: "hello", token: "a" });
    const hello = await b.next("hello_ok");
    assert.ok(hello.threads.find(row => row.id === thread.id).settled_at);
    assert.equal(hello.messages.length, 0, "thread history never leaks into coordinator hello");
