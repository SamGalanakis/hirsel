...
An Artifact is an explicitly published result with a stable global ID, positive revision and mutable content. It has no owning Thread. Agent tools create, edit, list and show artifacts within their Thread scope; explicit message, activity, material-state or validated brief references grant access. Create, edit and show place a compact reference card in the addressed conversation. Authorized references permit edits with current-content compare-and-swap; shared outputs are deliberately mutable across referencing Threads, without exposing those other conversations. Multiple Threads may reference the same artifact. Each edit advances the revision and every Task state that explicitly references it. Cards always resolve the latest content, including cards in earlier messages. Revisions detect change; Hirsel stores no historical artifact versions.
