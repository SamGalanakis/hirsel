...
describe("durable thread inventory", () => {
  ...
  it("merges replay and history pages without losing live messages or cross-owning citations", () => {
    ...
    const detail: ThreadDetail = { brief: { text: "", artifact_ids: [] }, thread: makeThread(), messages: [row(1, 1), row(2, 2, [1])], turns: [], turn_timelines: [], activities: [], related_items: [], grants: [], has_more: false };
...
it("does not replace a live current assignment with an older page response", () => {
  ...
  const merged = mergeDetail(prior, { thread: makeThread(1), brief: { text: "Old assignment", artifact_ids: [] }, messages: [], turns: [], turn_timelines: [], activities: [], related_items: [], grants: [], has_more: true }, true);
