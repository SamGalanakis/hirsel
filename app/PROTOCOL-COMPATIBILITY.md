# Hirsel wire compatibility and history appendix

> This is an archival, version-by-version account of retained wire behavior.
> The current canonical Task, global conversation, generated-action, and
> temporary-utility contract is [`PROTOCOL.md`](PROTOCOL.md). Historical
> `Chat`, `Ping`, side-session, and auto-resolution names below are
> compatibility spellings, not frontend destinations or interaction concepts.

`events`/`event_upsert` are Tasks, message `ref` is an Anchor, Task scope adds
an Event mention, and `event_action` drives generated-instrument transitions or
settlement. An action is not inherently terminal: an adaptive instrument may
advance while the Task stays open. `ping:<id>` is likewise a compatibility
placement for a Task View, not a visible Ping destination.

Transport-agnostic bidirectional message stream (ADR-0006). v1 carrier:
WebSocket, JSON text frames, one message per frame. This appendix records how
that compatibility surface accumulated; the live schema is implemented by the
Rust `hirsel-proto` crate and mirrored where needed by the PWA client.
...
Current Host note (2026-07-23): this entire section is opt-in compatibility.
Set `HIRSEL_COMPAT_SIDE_SESSIONS=1` before starting the Host. Without it,
legacy open frames are rejected explicitly and normal Host startup retains no
side-session backend and starts no reaper. With it, both the first side session
and its TTL reaper remain lazy until the first open frame.
...
{ "type": "confirm_conclusion", "sc", "text" }     // owner-edited final text
  → host posts the Owner's anchor-refed reply in the main thread (normal msg flow, normal agent
    enqueue, idempotency client_id "side-conclude:<sc>") without settling the Task, then discards
    the side session + transcript; only an explicit Event action settles a Task
...
The TTL is parsed only while `HIRSEL_COMPAT_SIDE_SESSIONS=1`.
