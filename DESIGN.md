...
### Navigation
- There is almost none by design — "the agent is the interface." Chat is the whole app. The only chrome is a thin `border-b` top bar (wordmark left; Processes icon + connection pill right). No tab bar, no sidebar, no settings sprawl.
