...
Schema 8 binds every push token to its authenticated device, restricts push
platforms to the supported values, and loads delivery targets only while that
device remains unrevoked. It retains schema 7's mutually exclusive
`threads.icon_blob_id` reference, Lash process delivery receipts, and Thread
authority. There is no in-place migration: schema 7 and all earlier layouts
