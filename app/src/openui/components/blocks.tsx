...
export const Text: OpenUiRenderer<{ text?: string; tone?: string; size?: string }> = p =>
  <p class={cn("min-w-0 break-words leading-relaxed", p.props.size === "sm" ? "text-meta" : "text-sm",
...
export const Callout: OpenUiRenderer<{ text?: string; variant?: string; title?: string }> = p =>
  <div role={p.props.variant === "danger" ? "alert" : undefined} data-slot="openui-callout" data-variant={p.props.variant ?? "info"}
    ...
    <span class="text-sm leading-relaxed">{p.props.text}</span>
  </div>;
...
export const CodeBlock: OpenUiRenderer<{ code?: string; language?: string }> = p =>
  <pre data-slot="openui-code" data-language={p.props.language} class="min-w-0 overflow-x-auto rounded-lg border border-border bg-muted/40 p-3 font-mono text-xs leading-relaxed"><code>{p.props.code}</code></pre>;
